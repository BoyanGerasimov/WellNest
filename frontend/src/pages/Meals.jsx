      ...
      <div className="grid grid-cols-1 gap-4">
        {meals.length === 0 ? (
          <div className="text-center py-12 bg-white rounded-lg shadow">
            <p className="text-gray-500 mb-4">No meals logged yet. Start tracking your nutrition!</p>
            <Link
              to="/meals/new"
                ...
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-4 gap-4 text-sm mb-3">
                    <div>
                      ...
                      <p className="font-semibold text-gray-900">{Math.round(meal.totalCalories)}</p>
                    </div>
                    <div>
                      <span className="text-gray-500">Protein</span>
                      <p className="font-semibold text-gray-900">{meal.totalProtein.toFixed(1)}g</p>
                    </div>
                    <div>
                      <span className="text-gray-500">Carbs</span>
                      <p className="font-semibold text-gray-900">{meal.totalCarbs.toFixed(1)}g</p>
                    </div>
                    <div>
                      <span className="text-gray-500">Fats</span>
                      <p className="font-semibold text-gray-900">{meal.totalFats.toFixed(1)}g</p>
                    </div>
                  </div>
                    ...
                    <div className="mt-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        {meal.foodItems.length > 5 && (
                          <span className="text-xs text-gray-500">+{meal.foodItems.length - 5} more</span>
                        )}
                      </div>
                    </div>
                  )}
                  {meal.notes && (
                    <p className="mt-3 text-sm text-gray-600 line-clamp-2">{meal.notes}</p>
                  )}
                </div>
